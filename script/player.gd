...
const GROUND_ACCEL := 50.0  # 地面加速
const GROUND_DECEL := 1000.0  # 地面减速
const AIR_ACCEL := 30.0  # 地面加速 * 0.5
...
func handle_movement(delta: float) -> void:
	...
	var accel = GROUND_ACCEL if is_on_floor() else AIR_ACCEL  #加速
	var decel = GROUND_DECEL if is_on_floor() else 0.0   #减速
